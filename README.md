# INDIA CPI-INFLATION-Data-Analysis (Interactive Dashboard Creation using MS Excel)
## Project Overview
This project analyzes India's Consumer Price Index (CPI) data to uncover inflation trends, category-wise contributions, food inflation drivers, and the impact of COVID-19 on key sectors. Using Microsoft Excel, the project applies data cleaning, inflation calculations, pivot tables, and dashboard development techniques to transform raw CPI data into actionable insights.

## Dataset Used
- <a href= "https://github.com/mithun-debug/CPI-INFLATION-/blob/main/All_India_Index_Upto_April23.xlsx">Dataset</a>

## Project Objective
- Analyze category-wise contribution to the CPI basket.
- Examine Year-over-Year (YoY) inflation trends from 2017 onwards.
- Evaluate Month-over-Month (MoM) food inflation trends during May 2022 – May 2023.
- Assess the impact of COVID-19 on inflation patterns across Food, Healthcare, and Essential Services.
- Build an interactive Excel dashboard for inflation monitoring and trend analysis.

## Data Cleaning & Preparation
- Handled missing values using appropriate imputation techniques.
- Removed incomplete records where necessary.
- Standardized date fields for time-series analysis.
- Verified category-level CPI consistency.
- Prepared data for pivot tables and dashboard reporting.

## Key Analyses Performed
1. CPI Basket Contribution Analysis
Calculated the percentage contribution of broad CPI categories by assuming equal category weights as specified in the case study.

Contribution% = (Category CPI/Total CPI of All Categories)*100

2. Year-over-Year (YoY) Inflation Analysis
Analyzed the long-term inflation trend using the General Index.

YoY Inflation% = (Current Year CPI − Previous Year CPI)/Previous Year CPI*100

3.Food Inflation Analysis
Studied Month-over-Month changes in Food & Beverages CPI between May 2022 and May 2023.

MoM Inflation% = (Current Month CPI − Previous Month CPI)/Previos Month CPI*100

4. Food Inflation Driver Analysis
Identified the food sub-category contributing most to food inflation.

Absolute Change = CPI May 2023 ​− CPI May 2022​

5. COVID-19 Impact Assessment
Compared inflation trends before and after the March 2020 lockdown across:

Food & Beverages
Healthcare
Fuel & Light (Essential Services)

## Dashboard
<img width="1541" height="713" alt="Screenshot_of_Dashboard" src="https://github.com/user-attachments/assets/e5ba2920-b205-414d-a228-2a552e92c78e" />


